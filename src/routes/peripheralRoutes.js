...
// GET http://localhost:3000/api/local-peripherals
// ==========================================
router.get('/', (req, res) => {
  db.all('SELECT * FROM local_peripherals', [], (err, rows) => {
    if (err) {
...
// POST http://localhost:3000/api/local-peripherals
// ==========================================
router.post('/', (req, res) => {
  ...
  const sql = 'INSERT INTO local_peripherals (glpi_id, name) VALUES (?, ?)';
  const params = [glpi_id || null, name|| ''];
...
// PUT http://localhost:3000/api/local-peripherals/:id
// ==========================================
router.put('/:id', (req, res) => {
  const { id } = req.params; // On récupère l'ID depuis l'URL
  const { glpi_id, name, notes } = req.body;

  const sql = 'UPDATE local_peripherals SET glpi_id = ?, name = ? WHERE id = ?';
...
// DELETE http://localhost:3000/api/local-peripherals/:id
// ==========================================
router.delete('/:id', (req, res) => {
  const { id } = req.params;

  const sql = 'DELETE FROM local_peripherals WHERE id = ?';
